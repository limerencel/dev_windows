...
public:
    ...
    bool LoadBitmap(std::string path) {
        ...
        if (hbmp == NULL) {
            ...
            width = bmp.bmWidth;
            height = bmp.bmHeight;
            // Always calculate frame dimensions for sprite sheets
            frameWidth = width / maxFrames;
            frameHeight = height / 4;  // 4 rows for different directions
            return true;
        }
    }
    ...
    void SetAnimate(bool shouldAnimate) { animate = shouldAnimate; }
    ...
    void RenderFrame(HDC hdc) {
        ...
        TransparentBlt(hdc, x, y, frameWidth, frameHeight, hdcMem, 
                      currentFrame * frameWidth, frameRow * frameHeight, 
                        frameWidth, frameHeight, bitmapBGColor);
        SelectObject(hdcMem, hbmpOld);  // Restore old bitmap before deleting DC
        DeleteDC(hdcMem);
    }
